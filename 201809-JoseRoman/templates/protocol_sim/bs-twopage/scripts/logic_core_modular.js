...
function traduceHTTPtoTCP(){
    ...
    if(http_array.length == 0){
        ...
        if(parseInt(tcpsizeOfObject)<= 0){
            ...
            /*document.getElementById("httpnObjects") = 1;
            document.getElementById("httpipserver") = tcpIpServer;
            document.getElementById("httpipclient") = tcpIpClient;
            document.getElementById("httpRequest") = "GET";
            document.getElementById("ipDestination") = tcpIpServer;
            document.getElementById("ipSource") = tcpIpClient;
            document.getElementById("ipTTL") = 64;
...
function tcpServerReaction(host, remote, globalParameters){
    ...
    switch(host.state){
        ...
        case "ESTABLISHED":
            //Connection established
            if(remote.toSend.length > 0 && host.received.length > 0){
                if(checkTimeServer(host.received[0].s_timestamp,actualT)){
                    if(host.received.length > 0 && host.received[0].dOffset > 0){
                        var tcp_packet = new TcpPacket(actualT,futureT,sPort,dPort,
                        host.actualSequence,host.received[0].seq,0,[0,1,0,0,0,0],host.window,
                        host.received[0].color,false);
                        remote.received.push(tcp_packet);
                        tcp_array.push(tcp_packet);
                        host.received.splice(0,1);
                    }else if(host.received.length > 0){
                        host.received.splice(0,1);
                    };
                }
            }else if(remote.toSend.length == 0 && host.received.length > 0){
                if(checkTimeServer(host.received[0].s_timestamp,actualT)){
                    if(host.received.length > 0 && host.received[0].dOffset > 0){
                        var tcp_packet = new TcpPacket(actualT,futureT,sPort,dPort,
                        host.actualSequence,host.received[0].seq,0,[0,1,0,0,0,0],host.window,
            ...
            }else if(host.toSend.length > 0){
                if(host.waitingForACK.length == 0 && host.received.length == 0 && host.toSend.length > 0){
                    if(parseInt(host.window) - host.waitingForACK.length > 0){
                        remote.received.push( markTimestampClient(host.toSend[0],actualT,futureT,host.window));
                        tcp_array.push( markTimestampClient(host.toSend[0],actualT,futureT,host.window));
                        host.waitingForACK.push(markTimestampClient(host.toSend[0],actualT,futureT,host.window));
                        host.timeouts.push = globalParameters.cTimeout;
                        host.toSend.splice(0,1);
                    };
                }else if(host.waitingForACK.length > 0 && host.received.length > 0 
                    && checkTimeClient(host.received[0].c_timestamp,actualT)){
                        if(host.received[0].ack < host.waitingForACK[0].seq){
                            remote.received.push(markTimestampClient(host.waitingForACK[0],actualT,futureT,host.window));
                            tcp_array.push(markTimestampClient(host.waitingForACK[0],actualT,futureT,host.window));
                            host.timeouts[0] = globalParameters.cTimeout;
                            host.window = 1;
                        }else if (host.received[0].ack > host.waitingForACK[0].seq){
                            while(host.waitingForACK[0].seq <= host.received[0].ack && host.waitingForACK.length > 0){
                                host.waitingForACK.splice(0,1);
                                host.timeouts.splice(0,1);
                            };
                            host.window++;
                            if(host.toSend.length > 0 && parseInt(host.window) - host.waitingForACK.length > 0){
                                remote.received.push( markTimestampClient(host.toSend[0],actualT,futureT,host.window));
                                tcp_array.push( markTimestampClient(host.toSend[0],actualT,futureT, host.window));
                                host.waitingForACK.push(markTimestampClient(host.toSend[0],actualT,futureT,host.window));
                                host.timeouts.push = globalParameters.cTimeout;
                ...
                }else if(host.waitingForACK.length > 0){
                    var counter = 0;
                    while(host.timeouts[counter] > 0 && counter > host.timeouts.length){
                        counter++; 
                    };
                    if(counter < host.timeouts.length){
                        remote.received.push(markTimestampClient(host.waitingForACK[counter],actualT,futureT,host.window));
                            host.timeouts[counter] = globalParameters.cTimeout;
                            host.window = 1;
                    };
                }
            }
...
function tcpClientReaction(host,remote,startConnection, globalParameters){
    ...
    var hWFA = host.waitingForACK;
    var r = remote;
    //Pass this to the parseTCP function, it's called everytime it's invoked
    host.toSend = objectToTcpArray(sPort,dPort,parseInt(seq) + 1,hDTT,host.actualDataOffset);
    ...
    switch(host.state){
        ...
        case "ESTABLISHED":
            ...
            }else if(host.toSend.length == 0 && remote.dataToTransfer <= 0 && host.received.length == 0/*remote.toSend == 0*/){
                var tcp_packet_sended = new TcpPacket(actualT,futureT,sPort,dPort,
                    seq,host.actualACK,0,[0,0,0,0,0,1],tcpServerWindow,getColor(),false);
                    r.received.push(tcp_packet_sended);
                    tcp_array.push(tcp_packet_sended);
                    ...
                    console.log("Client : All data transmited, closing connection");
                    host.state = "FIN_WAIT_1"
                    //Errors in ACK and Sequence Numbers in last packet
            }else if(host.toSend.length == 0 && host.waitingForACK.length == 0){
                //Add to condition below ACK delayed if necesary
                //host.dataToTransfer = 0 //DELETE WHEN SERVER WORKS WITH THE SAME ALGORITHM
                if(host.received.length > 0 && host.received[0].dOffset > 0){
                    var tcp_packet = new TcpPacket(actualT,futureT,sPort,dPort,
                    host.actualSequence,host.received[0].seq,0,[0,1,0,0,0,0],host.window,
                    host.received[0].color,false);
...
function autoFill(){
    ...
    sValue("tcpServerWindow", 3);
    sValue("tcpClientWindow", 3);
    sValue("tcpServerTimeout", 20);
    sValue("tcpClientTimeout", 20);
    //UDP Parameters
